...
if (isset($_GET['type']) && (isset($_GET['year'])) && (isset($_GET['make'])) && (isset($_GET['model']))) {
	...
	$query = "SELECT DISTINCT carsmake FROM cars WHERE LOWER(carstype) = LOWER('" . $type . "') AND '" . $year . "' Between carsYearMin and carsYearMax ORDER BY carsmake ASC";
	$result = mysqli_query($conn, $query);
	...
	$query = "SELECT Min(carsYearMin),Max(carsYearMax) FROM cars WHERE LOWER(carstype) = LOWER('" . $type . "') ORDER BY carsmake DESC";
	$result = mysqli_query($conn, $query);

	if (mysqli_num_rows($result) > 0) {
	  while($row = mysqli_fetch_assoc($result)) {
		$min = $row['Min(carsYearMin)'];
		$max = $row['Max(carsYearMax)'];
		for($i = "$max"; $i >= "$min"; $i--){
			$json[] = $i;
